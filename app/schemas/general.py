...
class TenderInfo(BaseModel):
    ...
    purchaseType: str = Field(
        default="Электронный аукцион", description="Способ определения поставщика"
    )
    ...
    deliveryInfo: Optional[DeliveryInfo] = Field(
        default_factory=DeliveryInfo, description="Информация о доставке"
    )
    paymentInfo: Optional[PaymentInfo] = Field(
        default_factory=PaymentInfo, description="Информация об оплате"
    )
